...
A live instance of this bot (nick `vjt-claude`) hangs out on [Azzurra IRC](https://azzurra.chat/). Join `#sniffo` via the webchat at **[azzurra.chat](https://azzurra.chat/)** (channel `#sniffo`) and say hi — no account or client setup required.
...
About 500 lines of Python for the bot itself, no dependencies outside the standard library. Two small sidecars live alongside it (see [Companions](#companions)).
...
Default config is inline at the top of `bot.py` — adjust `HOST`, `PORT`, `NICK` for your target network.
...
Everything in this section is about how the bot shapes the agent's experience on IRC — what events it sees, what it can do, and how human gatekeepers stay in the loop.
